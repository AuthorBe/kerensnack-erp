                ...
                <div style="display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:14px;padding-bottom:14px;border-bottom:1px solid var(--color-hairline);margin-bottom:2px;">
                    ...
                    <template x-if="selectedCustomer">
                        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                            ...
                            <template x-if="Number(selectedCustomer.grup_diskon_persen) > 0 || Number(selectedCustomer.grup_diskon_nominal) > 0">
                                <span class="badge" style="display:inline-flex;align-items:center;gap:4px;background:rgba(245,158,11,0.1);color:#f59e0b;border:1px solid rgba(245,158,11,0.25);font-weight:700;font-size:11px;padding:4px 10px;border-radius:999px;" :title="'Diskon bawaan grup ' + (selectedCustomer.nama_grup_harga || '')">
                                    <i data-lucide="percent" style="width:12px;height:12px;"></i>
                                    <span>Diskon Grup: </span>
                                    <span x-text="Number(selectedCustomer.grup_diskon_persen) > 0 ? (selectedCustomer.grup_diskon_persen + '%') : formatRupiah(selectedCustomer.grup_diskon_nominal)"></span>
                                </span>
                            </template>
...
function createSalesOrderApp() {
    ...
    return {
        ...
        get allowedPaymentOptions() {
            ...
            if (this.selectedCustomer.is_konsinyasi || this.selectedCustomer.tipe_pembayaran_default === 'konsinyasi') {
        ...
        getPaymentSchemeLabel(type) {
            const labels = {
                ...
                'tempo_faktur': 'Tempo Faktur (Kiriman Berikutnya)',
                'tempo_tanggal': 'Tempo Tanggal',
        ...
        getPriceForProduct(itemId) {
            ...
            // PILIHAN B (Strict Rejection): Level harga toko wajib terdaftar di /pricing
