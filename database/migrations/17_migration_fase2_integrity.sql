-- ============================================================================
-- Migrasi 17: Integritas Database & Proteksi Concurrency Konsinyasi
-- 1. Foreign Key kunjungan_konsinyasi.pesanan_id -> ON DELETE SET NULL
-- 2. Concurrency Safety: Tambahkan FOR UPDATE pada fn_proses_kunjungan_konsinyasi
-- ============================================================================

-- 1. Perbaiki constraint kunjungan_konsinyasi.pesanan_id
ALTER TABLE public.kunjungan_konsinyasi 
    DROP CONSTRAINT IF EXISTS kunjungan_konsinyasi_pesanan_id_fkey;

ALTER TABLE public.kunjungan_konsinyasi 
    ADD CONSTRAINT kunjungan_konsinyasi_pesanan_id_fkey 
    FOREIGN KEY (pesanan_id) REFERENCES public.pesanan(id) ON DELETE SET NULL;

-- 2. Perbarui fn_proses_kunjungan_konsinyasi dengan Row-Level Locking (FOR UPDATE)
